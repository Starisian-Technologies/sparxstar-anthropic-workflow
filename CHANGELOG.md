...
### Fixed
- Cross-repository callers failed in `build-context` with `not our ref` while checking out the platform reference docs: the step resolved its checkout ref from `github.workflow_ref`, which in a reusable-workflow call is the *caller's* top-level ref — on a `pull_request` run that is `refs/pull/<n>/merge`, a ref that exists only in the caller. The privileged job now pins checkout to `github.job_workflow_sha` (the commit SHA of this reusable workflow for the job), guaranteeing a valid ref in this repository. The fix preserves the privilege split — `build-context` still checks out only its own repo and the registries, never caller PR-head code. (Surfaced only cross-repo; same-repo self-tests passed because `refs/pull/<n>/merge` resolves locally.)
